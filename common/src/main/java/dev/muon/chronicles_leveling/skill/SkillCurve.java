 ...
 * expression on {@link dev.muon.chronicles_leveling.config.ConfigSync.SkillCurves};
 * {@code l} is bound to the current skill level.
 *
 * <p>Synced (it's part of {@code ConfigSync}), so server ops can retune curves
 * without a restart and clients see the right progress bar fill.
 *
...
public final class SkillCurve {
    ...
    public static int xpToNext(String skillId, int level) {
        if (level < 1) level = 1;
        ValidatedExpression expr = Configs.SYNC.skillCurves.curveFor(skillId);
        if (expr == null) return Math.max(1, (int) Math.round(FALLBACK_COST));
