...
/**
 * XP-cost-to-next-level math, per skill. Each skill section on
...
public final class SkillCurve {
    ...
    public static int xpToNext(String skillId, int level) {
        if (level < 1) level = 1;
        ConfigSkills.Skill section = Configs.SKILLS.section(skillId);
        if (section == null) return Math.max(1, (int) Math.round(FALLBACK_COST));
        ValidatedExpression expr = section.xpCurve;
